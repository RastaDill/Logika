...
class Game(object):
    ...
    def __init__(self):
        ...
        )  # group of blocks class Block (manages layers)
        self.old_block_list = (
            pygame.sprite.Group()
        )  # the group of blocks which can not be moved
        self.render_up_block = (
            False  # render other sprites over block (pipe_u and playinf_field_board)
        )
        ...
        self.xy_pos_bolt = [
            ...
            [623, 463],
            [2, 463],
        ]  # list of coordinates of four bolts
    ...
    def create_object_game_static(self):
        ...
        self.playing_field = basic.Sprite(
            os.path.join("image", "background", "playing_field.png"), 0, 0
        )
        self.playing_field_board = basic.Sprite(
            os.path.join("image", "background", "playing_field_board.png"), 0, 0
        )

        # Create button
        self.button_menu = basic.Button(
            os.path.join("image", "button", "menu_up_b.png"),
            os.path.join("image", "button", "menu_down_b.png"),
            self.x_drop * self.chance_box + 68,
            50,
        )
        self.button_restart = basic.Button(
            os.path.join("image", "button", "restart_up_b.png"),
            os.path.join("image", "button", "restart_down_b.png"),
            self.x_drop * self.chance_box + 113,
            50,
        )
        ...
        for i in range(len(basic.Block.block_image)):
            pipe_d = basic.Sprite(
                os.path.join("image", "pipe", "pipe_down.png"),
                self.x_block - 2,
                (self.y_block * i) + 25,
            )
            pipe_u = basic.Sprite(
                os.path.join("image", "pipe", "pipe_up.png"),
                self.x_block + 54,
                (self.y_block * i) + 25,
            )
        ...
        for i in range(self.chance):
            ...
            tumbler = basic.Tumbler(
                os.path.join("image", "tumbler", "tumbler_off.png"),
                os.path.join("image", "tumbler", "tumbler_on.png"),
                (self.x_drop * self.chance_box) + self.delta_x,
                (self.y_drop * i) + self.delta_y,
            )
            self.tumbler_list.append(tumbler)

            result = basic.Result(
                os.path.join("image", "pictograms", "gamma.png"),
                os.path.join("image", "pictograms", "omega.png"),
                os.path.join("image", "board", "result.png"),
                (self.x_drop * self.chance_box) + self.delta_x * 3,
                (self.y_drop * i) + self.delta_y,
            )
            ...
            for j in range(self.chance_box):
                drop = basic.Sprite(
                    os.path.join("image", "drop point", "drop_point.png"),
                    (self.x_drop * j) + self.delta_x,
                    (self.y_drop * i) + self.delta_y,
                )
                self.drop_list[i].append(drop)

        # Create object secret door, overall result, candy
        self.door_board = basic.Sprite(
            os.path.join("image", "door", "secret_door_board.png"), 15, 18
        )
        self.door = basic.Door(
            os.path.join("image", "door", "secret_door_top.png"),
        ...
        self.overall_result = basic.Sprite(
            os.path.join("image", "board", "overall_result.png"),
            self.x_drop * self.chance_box + 48,
            18,
        )

        # Create brain
        self.brain = basic.Brain(
            self.y_drop,
            self.block_list,
            self.old_block_list,
            self.drop_list,
    ...
    def create_object_game_dynamic(self):
        ...
        for i in range(len(basic.Block.block_image)):
            block = basic.Block(
                os.path.join("image", "blocks", basic.Block.block_image[i]),
                self.x_block,
                (self.y_block * i) + 27,
                i,
            )
    ...
    def create_object_menu(self):
        ...
        self.rule = basic.Description(
            os.path.join("image", "menu", "rule.png"), 641, 0, self.size
        )
        self.about = basic.Description(
            os.path.join("image", "menu", "about.png"), 0, 481, self.size
        )

        self.button_left = basic.Button(
            os.path.join("image", "button", "left_up_b.png"),
            os.path.join("image", "button", "left_down_b.png"),
            157,
            320,
        )
        self.button_right = basic.Button(
            os.path.join("image", "button", "right_up_b.png"),
            os.path.join("image", "button", "right_down_b.png"),
            455,
            320,
        )
        self.button_enter = basic.Button(
            os.path.join("image", "button", "enter_up_b.png"),
            os.path.join("image", "button", "enter_down_b.png"),
            self.x_drop * self.chance_box + 85,
            100,
        )
        ...
        for i in range(4):
            bolt = basic.Bolt(
                os.path.join("image", "menu", "bolt.png"), self.xy_pos_bolt[i]
            )
    ...
    def run_rule_brain(self):
        ...
        if not self.end_game:
            self.line, self.end_game, self.winning = self.brain.check_block_line(
                self.line, self.secret_comb, self.door
            )
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for block in self.block_list:
                ...
                if block.check_mouse(self.m_pos):
                    ...
                    self.block_list.move_to_front(
                        block
                    )  # roaming sprite at the forefront
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    sys.exit()  # if click on the cross button window
