...
class Sprite(pygame.sprite.Sprite):
    ...
    def __init__(self, name_image, x_pos, y_pos):
        ...
        self.state = (
            None  # specifies the state of the object OR writes a special variable
        )
        self.image = pygame.image.load(
            name_image
        ).convert_alpha()  # load and conver image
    ...
    def check_mouse(self, m_pos, change_state=True):
        ...
        if (
            (self.rect.left < m_pos[0] < self.rect.right)
...
class Candy(Sprite):
    ...
    def __init__(self, name_image, x_pos, y_pos):
        super(Candy, self).__init__(name_image=name_image, x_pos=x_pos, y_pos=y_pos)
...
class Tumbler(Sprite):
    ...
    def __init__(self, name_image, name_image_next, x_pos, y_pos):
        super(Tumbler, self).__init__(name_image=name_image, x_pos=x_pos, y_pos=y_pos)
...
class Door(Tumbler):
    ...
    def update(self, screen):
        ...
        screen.blit(
            self.image, [self.rect.x, self.rect.y - self.count_frame]
        )  # first path door
        screen.blit(
            self.image_next, [self.rect.x, self.rect.y + 12 + self.count_frame]
        )  # second path door
...
class Button(Sprite):
    ...
    def __init__(self, name_image, name_image_next, x_pos, y_pos):
        super(Button, self).__init__(name_image=name_image, x_pos=x_pos, y_pos=y_pos)
...
class Result(Sprite):
    ...
    def __init__(self, name_gamma, name_omega, name_miniscreen, x_pos, y_pos):
        super(Result, self).__init__(name_image=name_gamma, x_pos=x_pos, y_pos=y_pos)
    ...
    def update(self, screen):
        ...
        screen.blit(
            self.image_miniscreen, [self.rect.x + 23, self.rect.y]
        )  # self.rect.x=400
...
class Block(Sprite):
    ...
    def __init__(self, name_image, x_pos, y_pos, id_number, x_out=710):
        super(Block, self).__init__(name_image=name_image, x_pos=x_out, y_pos=y_pos)
    ...
    def check_point(self, drop_list, line):
        ...
        if self.state == 4:
            ...
            for j in range(len(drop_list[line])):
                if (
                    pygame.sprite.collide_rect(drop_list[line][j], self)
                    and count_cross < 2
                ):
                    count_cross += 1
...
class Brain(object):
    ...
    def __init__(
        self, y_drop, block_list, old_block_list, drop_list, tumbler_list, game
    ):
        self.font = pygame.font.Font(
            os.path.join("font", "DejaVuSansMono.ttf"), 16
        )  # font for balance
        self.font_state = pygame.font.Font(
            os.path.join("font", "DejaVuSansMono.ttf"), 25
        )  # font for state
        self.balance_gamma = self.font.render(
            "", True, [24, 240, 24]
        )  # enter text on screen about number bulls (gamma)
        self.balance_omega = self.font.render(
            "", True, [24, 240, 24]
        )  # enter text on screen about number cows (omega)
        self.text_state = self.font_state.render(
            "-------", True, [255, 255, 0]
        )  # enter text on screen YOU WIN or YOU LOSE
    ...
    def __time(self):
        ...
        self.text_state = self.font_state.render(
            (str(self.minutes) + ":" + str(self.seconds)), True, [255, 255, 0]
        )
    ...
    def update(self, screen):
        ...
        for i in range(0, len(self.balance_list), 2):
            screen.blit(
                self.balance_list[i], [430, (self.y * i * 0.5) + 91]
            )  # the x coordinate almost is the same as the sprite miniscreen
...
class TextMenu(pygame.sprite.Sprite):
    ...
    var_text = {0: "New Game", 1: "Rules", 2: "Survival", 3: "  Exit", 4: "Credits"}
    ...
    def __init__(self, x_pos, y_pos):
        ...
        self.image = self.font.render(
            TextMenu.var_text[self.__current_text], True, self.color
...
class Description(Sprite):
    ...
    def __init__(self, name_image, x_pos, y_pos, screen_size):
        super(Description, self).__init__(
            name_image=name_image, x_pos=x_pos, y_pos=y_pos
        )
        self.state = False
        self.count_x = -1
        self.count_y = -1
    ...
    def update(self, screen):
        self.__check_move()
        screen.blit(
            self.image, [self.rect.x + self.count_x, self.rect.y + self.count_y]
        )
...
class Bolt(Sprite):
    ...
    def __init__(self, name_image, xy_pos):
        super(Bolt, self).__init__(
            name_image=name_image, x_pos=xy_pos[0], y_pos=xy_pos[1]
        )
        ...
        if direction:
            ...
            if self.count_step < 360:
                ...
                self.image = pygame.transform.rotate(
                    self.org_image, self.angle + self.count_step
                )
                if not self.state:
